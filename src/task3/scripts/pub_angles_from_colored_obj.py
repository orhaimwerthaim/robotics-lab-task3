...
class pub_angles_from_colored_obj:
    '''this node gets the the images from the usb-cam of the bot and publishes the
    location of the blue object relative to the center of the frame'''
    def set_colored_object_center(self):
        ...
        if not cnt is None and len(cnt) > 0:
            ...
            if maxSize > 50:#we need to ignore (false positives) small objects (there could be some small blue objects in the room)
            #M = cv.moments(cnt[0])
                cX = int(M["m10"] / M["m00"])
